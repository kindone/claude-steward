...
### Core UX
- [ ] **Multi-client session sync** — when two clients are open on the same session, only the sending client sees new messages in real time; the idle client stays stale until manual refresh or app foreground. Fix: make the watch channel persistent — server sends `event: updated` (without closing the connection) each time a message is finalized; `ChatWindow` subscribes on mount for every session (not just pending ones) and re-fetches on each `updated` event. Pure push, no polling. Requires splitting `notifyWatchers` into a one-shot terminal path (current `done` for streaming recovery) and a persistent broadcast path (new `updated` for multi-client sync).
