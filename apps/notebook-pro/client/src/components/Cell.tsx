...
import type { Cell as CellType, Language, OutputItem, RichOutputKind, CompileResult } from '../types'
...
export function Cell({ cell, onUpdate, onDelete, onMoveUp, onMoveDown, canMoveUp, canMoveDown, isRunningByAI }: Props) {
  ...
  useEffect(() => {
    if (!editingName) setCellName(cell.name ?? '')
  }, [cell.name])
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      streamKernelRun(
        cell.id,
        (line) => setOutputs(prev => [...prev, { kind: 'text', text: line }]),
        (ok, output) => setCompileResult({ ok, output }),
        () => { setIsRunning(false); abortRef.current = null; onDone() },
        (msg) => {
          setOutputs(prev => [...prev, { kind: 'text', text: msg, isError: true }])
          setIsRunning(false)
          abortRef.current = null
          onDone()
        },
        (kind, payload) => setOutputs(prev => [...prev, { kind: kind as RichOutputKind, payload }]),
        ac.signal,
      )
    }
  ...
  // Fold-all / expand-all from toolbar
  useEffect(() => {
    const handler = (e: Event) => setIsFolded((e as CustomEvent<boolean>).detail)
    window.addEventListener('notebook:fold-all', handler)
    return () => window.removeEventListener('notebook:fold-all', handler)
  }, [])

  // AI-triggered run: mirror kernel output broadcast from chat SSE
  useEffect(() => {
    type RunEvent = { cellId: string; type: string; line?: string; kind?: string; payload?: string; ok?: boolean; output?: string; message?: string }
    const handler = (e: Event) => {
      const d = (e as CustomEvent<RunEvent>).detail
      if (d.cellId !== cell.id) return
      if (d.type === 'started') {
        setIsRunning(true)
        setOutputs([])
        setCompileResult(null)
      } else if (d.type === 'output') {
        setOutputs(prev => [...prev, { kind: 'text', text: d.line ?? '' }])
      } else if (d.type === 'rich_output') {
        setOutputs(prev => [...prev, { kind: d.kind as RichOutputKind, payload: d.payload ?? '' }])
      } else if (d.type === 'compile') {
        setCompileResult({ ok: d.ok ?? false, output: d.output ?? '' })
      } else if (d.type === 'done') {
        setIsRunning(false)
      } else if (d.type === 'error') {
        setOutputs(prev => [...prev, { kind: 'text', text: d.message ?? 'Error', isError: true }])
        setIsRunning(false)
      }
    }
    window.addEventListener('notebook:cell-run-event', handler)
    return () => window.removeEventListener('notebook:cell-run-event', handler)
  }, [cell.id])
  ...
  const handleSaveName = useCallback(async (val: string) => {
    setEditingName(false)
    const trimmed = val.trim()
    const newName = trimmed || null
    if (newName === (cell.name ?? null)) return
    const updated = await updateCell(cell.id, { name: newName })
    onUpdate(updated)
  }, [cell.id, cell.name, onUpdate])
      ...
      <div className={`rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden ${highlight}`}>
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 border-b border-[var(--color-border)] bg-black/20">
          ...
          <button
            ...
            className="text-[var(--color-muted)] hover:text-white leading-none text-[10px] px-0.5 transition-transform"
            title={isFolded ? 'Expand cell' : 'Collapse cell'}
            style={{ transform: isFolded ? 'rotate(-90deg)' : 'rotate(0deg)', transition: 'transform 150ms' }}
          >▼</button>
          ...
          {editingName ? (
            <input
              ...
              onKeyDown={e => {
                if (e.key === 'Enter') { e.preventDefault(); handleSaveName(cellName) }
                if (e.key === 'Escape') { setEditingName(false); setCellName(cell.name ?? '') }
              }}
              placeholder="cell name…"
              className="text-xs bg-transparent border-b border-[var(--color-accent)] outline-none text-white w-48 min-w-0"
              autoFocus
            />
          ) : (
            <button
              onClick={() => { setEditingName(true); setTimeout(() => nameInputRef.current?.select(), 0) }}
              className="text-xs text-[var(--color-muted)] hover:text-white truncate max-w-[200px] text-left"
              title={cellName || 'Click to name this cell'}
            >
              {cellName || <span className="opacity-40 italic">name…</span>}
            </button>
          )}
        ...
        {/* Output */}
        {outputs.length > 0 && (
          <div className="border-t border-[var(--color-border)] bg-black/30 p-3 space-y-2">
            {outputs.map((item, i) =>
              item.kind === 'text' ? (
                <pre key={i} className={`text-xs leading-relaxed whitespace-pre-wrap ${item.isError ? 'text-red-400' : 'text-gray-300'}`}>
                  {item.text}
                </pre>
              ) : (
                <RichOutput key={i} kind={item.kind} payload={item.payload} />
              )
            )}
          </div>
        )}
