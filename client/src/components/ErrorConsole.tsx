    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center">
          ...
          {/* Panel */}
          <div className="relative w-full sm:max-w-2xl max-h-[80vh] flex flex-col bg-[#111] border border-[#2a2a2a] sm:rounded-xl shadow-2xl overflow-hidden">
            {/* Header */}
            <div className="flex items-center gap-3 px-4 py-3 border-b border-[#1e1e1e] flex-shrink-0">
              ...
              <button onClick={() => setOpen(false)} className="text-[#555] hover:text-white text-lg leading-none ml-1">✕</button>
            </div>
  ...
  return (
    <div className="px-4 py-3 flex flex-col gap-1.5">
      <div className="flex items-start gap-2">
        <span className="text-[10px] text-[#444] font-mono flex-shrink-0 mt-0.5">{formatTime(err.timestamp)}</span>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button
            ...
            className="text-[#444] hover:text-[#888] leading-none text-base"
            title="Dismiss"
          >✕</button>
        </div>
      </div>
      {expanded && err.stack && (
        <pre className="text-[10px] text-[#555] bg-[#0a0a0a] border border-[#1e1e1e] rounded px-3 py-2 overflow-x-auto whitespace-pre-wrap break-all leading-relaxed max-h-[300px] overflow-y-auto">
          {err.stack}
        </pre>
      )}
    </div>
