...
- **Memory-safe builds.** The instance has ~4GB RAM and a 2GB swapfile. Repeated `npm install` + build cycles can exhaust memory and freeze the system (observed: `systemd-journald: Under memory pressure`). Rules: (1) diagnose dep issues first, then do **one** clean `npm install --include=dev`; never retry blindly. (2) Run `npm run build --workspace=client` and `--workspace=server` **sequentially**, not the combined `npm run build` which runs both concurrently. (3) Avoid running builds while Claude CLI processes are active when possible.
- **Rate-limit recovery.** When the user hints at hitting a usage/rate limit ("we hit the limit again", "recover from the db", "what were we working on"), run `node scripts/recover.mjs` first. It defaults to the most recently updated session and prints: limit-hit timestamps, any unanswered user prompt (the one that the limit cut off), and the last 6 messages. Pass a session ID to query a different one, or `--full` for un-truncated content. This is the fastest way to reload context without re-asking the user what we were doing.
...
1. **`CLAUDECODE=1` causes hanging** — child inherits this var from a parent Claude session and waits for IPC that never comes. Strip all env vars starting with `CLAUDE` from the spawn env, then re-admit the explicit allowlist: `CLAUDE_CODE_OAUTH_TOKEN` (long-lived headless-auth token from `claude setup-token`) and `ANTHROPIC_BASE_URL` (endpoint override). See `server/src/claude/process.ts` and `server/src/worker/job-manager.ts`.
