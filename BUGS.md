...
- [x] **No push when app is backgrounded (iOS)** — iOS Safari keeps the SSE connection alive when the app is backgrounded, so `clientDisconnectedEarly` was never set and push notifications were skipped. Fixed by adding foreground/background visibility tracking: client reports `visibilitychange` via `POST /api/events/visibility` (using `sendBeacon` for hidden), and `hasActiveClients()` now checks foreground connections only. Push condition in chat route changed from `clientDisconnectedEarly` to `clientDisconnectedEarly || !hasActiveClients()`.
- [x] **Push fires while app is in foreground** — when the app was open and a different session completed, both a push notification and an in-app toast would fire. Fixed by branching on `hasActiveClients()`: foreground → SSE `pushTarget` event (in-app toast), backgrounded/closed → web-push notification. Applied to all three push paths: `chat.ts`, `scheduler.ts`, `schedules.ts`.
