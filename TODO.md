...
- [x] **Tool activity indicator shows tool name** — streaming indicator reads `content_block_start.content_block.name` and displays the actual tool name (e.g. "Read", "Edit", "Bash", "WebSearch"); stale tool name after Stop/error is now cleared on the next send and on Stop so it never bleeds into a subsequent message
