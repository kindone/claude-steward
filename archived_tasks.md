...
- [x] **Tool activity indicator** — streaming indicator reads `content_block_start.content_block.name` and displays the actual tool name (Bash, Read, Edit, WebSearch, etc.); clears on first `text_delta` so it only shows while a tool is actively executing
