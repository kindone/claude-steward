...
- [x] **Tool activity indicator shows tool name** — streaming indicator reads `content_block_start.content_block.name` and displays the actual tool name (e.g. "Read", "Edit", "Bash", "WebSearch"); cleared on the first `text_delta` when Claude starts writing its response
