...
const RUNNER = path.resolve(__dirname, '../../../apps/notebook/src/runners/node_runner.js')

export class NodeKernel implements IKernel {
  ...
  private spawn(): void {
    ...
    this.child.on('exit', () => {
      ...
      for (const [cellId, { reject }] of this.pendingResolvers) {
        reject(new Error(`Node kernel exited while running cell ${cellId}`))
      }
      this.pendingResolvers.clear()
    })

    console.log(`[node-kernel] spawned pid=${this._pid} cwd=${this.cwd}`)
  }
