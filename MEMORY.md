...
- **Dev uses HMR (`vite dev`), prod uses built static files.** nginx routes `dev.steward.jradoo.com → :5173` (Vite dev server). WebSocket upgrades are forwarded by nginx (`ws-map.conf`), so HMR works over WSS. Vite proxies `/api → :3002` internally. nginx routes `steward.jradoo.com → :3001` (prod, built files). Do NOT set `build.watch` to non-null in `vite.config.ts` unconditionally — that activates watch mode for all builds including production.
