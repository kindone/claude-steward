...
// Copy display.py to DATA_DIR root so Python cells can `from display import vega, table, ...`
// Python kernel spawns with cwd=DATA_DIR, so the import resolves immediately.
const displayPySrc = path.join(__dirname, 'runners', 'display.py')
const displayPyDst = path.join(DATA_DIR, 'display.py')
if (fs.existsSync(displayPySrc)) fs.copyFileSync(displayPySrc, displayPyDst)
