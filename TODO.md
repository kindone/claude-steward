...
- [x] **Terminal access panel** — "Term" tab in the sidebar; xterm.js renders ANSI output; `POST /api/projects/:id/exec` SSE endpoint spawns `sh -c` with `FORCE_COLOR=1 / COLORTERM=truecolor`; 60 s timeout; up/down arrow history; Stop button; Clear button; tab panel stays mounted (CSS `hidden`) so terminal history persists across tab switches; `node-pty` deferred — add when interactive programs are needed
...
- [x] **Resume-poll ceiling** — replaced the 60-tick polling loop with a `GET /api/sessions/:id/watch` SSE endpoint; server sends `event: done` the instant `onComplete` writes the assistant message to the DB; 30 s keepalive pings prevent nginx idle timeout; `watchSession()` client helper uses native `EventSource`; no ceiling, instant update regardless of how long Claude runs
...
- [x] **File editor** — Edit button in viewer header toggles a full-height monospace `<textarea>`; Save / Cancel buttons; `●` dirty indicator; Cmd+S / Ctrl+S shortcut; `PATCH /api/projects/:id/files` writes atomically via `.steward-tmp` → `rename`; optimistic locking: server compares `lastModified` mtime and returns `409 Conflict` if the file changed; conflict banner shows **Overwrite** / **Reload file** options; Escape auto-cancels only when no unsaved changes
