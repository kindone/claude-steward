...
// Runners live in apps/notebook/src/runners/ — resolve from monorepo root
const RUNNER = path.resolve(__dirname, '../../../apps/notebook/src/runners/kernel_runner.py')
...
export class PythonKernel implements IKernel {
  ...
  private spawn(): void {
    ...
    console.log(`[python-kernel] spawned pid=${this._pid} cwd=${this.cwd}`)
  }
  ...
  async reset(): Promise<void> {
    ...
    return new Promise((resolve) => {
      ...
      const onLine = (line: string) => {
        if (line.trim() === 'RESET_DONE') { rl.close(); resolve() }
      }
      rl.on('line', onLine)
